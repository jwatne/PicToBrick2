...
public class Calculator {
	...
	public Lab rgbToLab(final Color rgb) {
		...
		// ----------------------------------------------------
		final Lab lab = new Lab((116.0 * y) - 16.0,
				500.0 * (x - y),
				200.0 * (y - z));
		return lab;
	}
	 ...
	 */
	public BufferedImage scale(final BufferedImage image, final int destinationWidth, final int destinationHeight,
			final int interpolation) {
		final double factorX = ((double) destinationWidth / (double) image.getWidth());
		final double factorY = ((double) destinationHeight / (double) image.getHeight());
		final BufferedImage imageScaled = new BufferedImage(destinationWidth, destinationHeight, image.getType());
	...
	public int[][][] pixelMatrix(final BufferedImage image) {
		...
		final int pixelMatrix[][][] = new int[imageHeight][imageWidth][3];
		final PixelGrabber grabber = new PixelGrabber(image, 0, 0, imageWidth, imageHeight, pixels, 0, imageWidth);
		try {
			grabber.grabPixels();
	 ...
	 */
	public Vector<Integer> randomCoordinates(final int width, final int height) {
	 ...
	 */
	public Vector<Integer> hilbertCoordinates(final int mosaicWidth, final int mosaicHeight) {
	 ...
	 * @return hilbert coordinates as vector
	 */
	private void hilbert(final int type, final int x, final int y, final int length, int n,
