...
public class OutputFileGenerator {
	 ...
	 * @author Tobias Reichling
	 */
	public OutputFileGenerator(final DataProcessor dataProcessing) {
	...
	public String generateDocuments(final boolean grafic, final boolean configuration, final boolean material,
		...
		final int width2 = (int) (100
				* (dataProcessing.getCurrentConfiguration().getBasisWidthMM() * dataProcessing.getMosaicWidth()));
		final int height2 = (int) (100 * (dataProcessing.getCurrentConfiguration().getBasisWidthMM()
				/ dataProcessing.getCurrentConfiguration().getBasisWidth()
				* dataProcessing.getCurrentConfiguration().getBasisHeight() * dataProcessing.getMosaicHeight()));
		...
		if (grafic) {
			...
			if (grafic) {
				graficString.append(menu_grafic);
			}
			;
			if (configuration) {
				graficString.append(menu_configuration);
			}
			;
			if (material) {
				graficString.append(menu_material);
			}
			;
			if (instruction) {
				graficString.append(menu_instruction);
			}
			;
			if (xml) {
				graficString.append(menu_xml);
			}
			;
			...
			final int height = (int) (mosaikImage.getHeight() * (600.0 / mosaikImage.getWidth()));
			final int width = 600;
			final int print_width = (int) (100
					* (dataProcessing.getMosaicWidth() * dataProcessing.getCurrentConfiguration().getBasisWidthMM()));
			...
			graficString
					.append((print_width / 100) + textbundle.getString("output_decimalPoint") + (print_width % 100));
		...
		if (configuration) {
			...
			if (grafic) {
				configurationString.append(menu_grafic);
			}
			;
			if (configuration) {
				configurationString.append(menu_configuration);
			}
			;
			if (material) {
				configurationString.append(menu_material);
			}
			;
			if (instruction) {
				configurationString.append(menu_instruction);
			}
			;
			if (xml) {
				configurationString.append(menu_xml);
			}
			;
			...
			for (final Enumeration<ElementObject> elementsEnumeration = dataProcessing.getCurrentConfiguration()
				...
				flag1++;
				elementsString.append("<img src=\"" + "element_" + counter + ".jpg\" width=\"" + width * 8
						+ "\" height=\"" + height * element.getHeight() + "\" alt=\"mosaic\" />");
			...
			if (grafic) {
				colorsString.append(menu_grafic);
			}

			if (configuration) {
				colorsString.append(menu_configuration);
			}

			if (material) {
				colorsString.append(menu_material);
			}

			if (instruction) {
				colorsString.append(menu_instruction);
			}

			if (xml) {
				colorsString.append(menu_xml);
			}
			...
			for (final Enumeration<ColorObject> colorsEnumeration = dataProcessing.getCurrentConfiguration()
				...
				colorsString.append(
						color.getRGB().getRed() + ", " + color.getRGB().getGreen() + ", " + color.getRGB().getBlue());
		...
		if (material) {
			...
			if (grafic) {
				billofmaterialString.append(menu_grafic);
			}

			if (configuration) {
				billofmaterialString.append(menu_configuration);
			}

			if (material) {
				billofmaterialString.append(menu_material);
			}

			if (instruction) {
				billofmaterialString.append(menu_instruction);
			}

			if (xml) {
				billofmaterialString.append(menu_xml);
			}
			...
			// count elements
			final List<List<Vector<String>>> mosaicMatrix = dataProcessing.getMosaic();
			...
			while (colorsEnumeration.hasMoreElements()) {
				...
				while (elementsEnumeration.hasMoreElements()) {
					...
					if (quantity > 0) {
						...
						billofmaterialString.append(material_between);
						billofmaterialString
								.append(quantity + " " + textbundle.getString("output_outputFiles_39") + "");
		...
		// instruction.html
		if (instruction) {
			List<List<Vector<String>>> mosaicMatrix = dataProcessing.getMosaic();
			final StringBuffer instructionString = new StringBuffer();
			instructionString.append(head);
			instructionString.append(menu_start);

			if (grafic) {
				instructionString.append(menu_grafic);
			}

			if (configuration) {
				instructionString.append(menu_configuration);
			}

			if (material) {
				instructionString.append(menu_material);
			}

			if (instruction) {
				instructionString.append(menu_instruction);
			}

			if (xml) {
				instructionString.append(menu_xml);
			}
			...
			for (int row = 0; row < dataProcessing.getMosaicHeight(); row++) {
				for (int column = 0; column < dataProcessing.getMosaicWidth(); column++) {
					...
					instructionString.append(textbundle.getString("output_outputFiles_40") + " " + (row + 1) + ", "
							+ textbundle.getString("output_outputFiles_41") + " " + (column + 1) + ": ");

					if (mosaicMatrix.get(row).get(column).isEmpty()) {
						instructionString.append("(" + textbundle.getString("output_outputFiles_42") + ")");
					} else {
						instructionString.append((String) mosaicMatrix.get(row).get(column).elementAt(0));
						instructionString.append(", ");
						instructionString.append((String) mosaicMatrix.get(row).get(column).elementAt(1));
					}
		...
		if (xml) {
			...
			if (grafic) {
				xmlAusgabe.append(menu_grafic);
			}

			if (configuration) {
				xmlAusgabe.append(menu_configuration);
			}

			if (material) {
				xmlAusgabe.append(menu_material);
			}

			if (instruction) {
				xmlAusgabe.append(menu_instruction);
			}

			if (xml) {
				xmlAusgabe.append(menu_xml);
			}
			...
			for (int row = 0; row < dataProcessing.getMosaicHeight(); row++) {
				...
				for (int column = 0; column < dataProcessing.getMosaicWidth(); column++) {
					if (mosaicMatrix.get(row).get(column).isEmpty()) {
						...
						xmldocumentString.append(mosaicMatrix.get(row).get(column).elementAt(0));
						xmldocumentString.append(xml_document_element_center);
						xmldocumentString.append(mosaicMatrix.get(row).get(column).elementAt(1));
