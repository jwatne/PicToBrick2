	...
	public Mosaic quantisation(BufferedImage image,
	    ...
	    for (int rows = 1; rows < mosaicHeight; rows++){
	    	for (int column = 1; column < mosaicWidth; column++){
	    		...
	    		if (currentColorName.equals(workingMosaic[rows+1][column])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows-1][column])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows][column+1])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows][column-1])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows+1][column+1])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows-1][column-1])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows+1][column-1])){colorCounter++;}
	    		if (currentColorName.equals(workingMosaic[rows-1][column+1])){colorCounter++;}
	    		    	...
	    		    	for (int windowColumn = (column - 1); windowColumn < (column + 2); windowColumn++){
	    		    		...
	    		    		for (int windowRow2 = (rows - 1); windowRow2 < (rows + 2); windowRow2++){
	    	    		    	for (int windowColumn2 = (column - 1); windowColumn2 < (column + 2); windowColumn2++){
	    	    		    		if (testColor.equals(workingMosaic[windowRow2][windowColumn2])){colorCounter2++;}
	    	    		    	}
	    	    		    }
	    		    		if (colorCounter2 > flag){newColor = testColor; flag = colorCounter2;};
	    		    	}
	    		    }
	    			//change color
