...
public class MoldingOptimizer
	...
	public Mosaic tiling(final int mosaicWidth,
		...
		final Configuration ministeckSystem = dataProcessing.getSystemConfiguration(2);
				...
				SwingUtilities.invokeAndWait(new Runnable() {
					public void run() {
						dataProcessing.errorDialog(textbundle.getString("output_moldingOptimisation_1") + "\n\r"
								+ textbundle.getString("output_moldingOptimisation_2"));
					}
				});
			...
			for (colorRow = 0; colorRow < mosaicHeight; colorRow++) {
				...
				for (int colorColumn = 0; colorColumn < mosaicWidth; colorColumn++) {
					...
					if (!pixel.isEmpty()) {
						...
						final Vector<ElementObject> elementsSorted = sortElementsByAvailability(
						...
						if (optimisationMethod == 1) {
							final Vector<ElementObject> criticalElements = new Vector<>();
							...
							if (criticalElements.size() > 0) {
								...
								while (criticalElementsEnum.hasMoreElements() && !elementSet) {
									...
									if (((colorRow + currentElement.getHeight() - 1) < mosaicHeight)
											...
											// right
											&& ((colorColumn
													+ (currentElement.getWidth() - (left + 1))) < mosaicWidth)) {
										// color matching
										// -------------------------
											...
											for (int elementColumn = 0; elementColumn < currentElement
												...
												if (currentElement.getMatrix()[elementRow][elementColumn]) {
													pixel2 = mosaic.getMosaic().get(colorRow
															+ elementRow).get((colorColumn + elementColumn) - left);
							...
							// ... or ...
							// if: additional optimisation mode, but no critical element is set to the
							// mosaic:
							// ...
	...
	private Vector<ElementObject> sortElementsByAvailability(final Enumeration elementsUnsorted) {
		...
		// -------------------
		final double singleAppearanceAverage = ((elementArray[currentColorNumber][5] * 2.0
				+ elementArray[currentColorNumber][6] * 20.0)
				/ (elementArray[currentColorNumber][5]
						+ elementArray[currentColorNumber][6]));
		final double singlePriceAverage = ((elementArray[currentColorNumber][5] * (27.0 / 114.0)
		...
		// the more 1er moldings we use the dearer are the covering costs of an 1x1
		// element
		//
		// complete fraction
	 ...
	 * @author Tobias Reichling
	 */
	private Enumeration<ElementObject> computeElements(final int covering, final int width, final int height) {
	 ...
	 * @author Tobias Reichling
	 */
	private void updateElementAppearance(final ElementObject element, final int counter) {
		final Random random = new Random();
		final int quantity = elementArray[currentColorNumber][computeElementNumber(element)];
		// decrement
		...
		// critical: quantity is bigger than the quantity of this element in a normal
		// molding
		// - solution: random usage of the critical elements
		// (random: to avoid visual artefacts !!!)
		// very critical: quantity is 2x bigger than the quantity of this element in a
		// normal molding
	...
	private void output() {
		...
		if (optimisationMethod == 0) {
			...
			final int recolored = (int) ((((double) pixelColorChanges) / ((double) (pixelCount))) * 1000);
			dataProcessing.setInfo("(" + textbundle.getString("output_moldingOptimisation_5") + ". " + recolored / 10
					+ textbundle.getString("output_decimalPoint") + recolored % 10 + " % "
					+ textbundle.getString("output_moldingOptimisation_6") + ".)", 3);
		}
		...
		dataProcessing.setInfo("[ " + textbundle.getString("output_moldingOptimisation_9") + " / "
				+ textbundle.getString("output_moldingOptimisation_10") + " ]", 2);
		// for each color ...
		for (int j = 0; j < colorCount; j++) {
			...
			if (elementArray[j][5] > 0 || elementArray[j][6] > 0) {
				...
				// decrement the remaining elements so often (with the quantity of a normal
				// molding)
		...
		final int percentUsage = (int) (((double) (normalFracturedAllColors + singleFracturedAllColors))
				/ ((double) (normalUsageAllColors + singleUsageAllColors)) * 10000.0);
		// output costs, etc.
		dataProcessing.setInfo(textbundle.getString("output_moldingOptimisation_11") + ": "
				...
				+ " (" + textbundle.getString("output_moldingOptimisation_13") + ". " + (percentUsage / 100)
				+ textbundle.getString("output_decimalPoint") + (percentUsage % 100) + " %) "
				+ textbundle.getString("output_moldingOptimisation_14") + ".", 3);
		dataProcessing.setInfo(textbundle.getString("output_moldingOptimisation_15") + " " + normalUsageAllColors + " "
				+ textbundle.getString("output_moldingOptimisation_16") + " " + singleUsageAllColors
				+ " " + textbundle.getString("output_moldingOptimisation_17") + ": " + euro
				+ textbundle.getString("output_decimalPoint") + centText + " "
				+ textbundle.getString("output_moldingOptimisation_18"), 3);
	}
	 ...
	 */
	private boolean elementEqual(final boolean[][] matrix1, final boolean[][] matrix2, final int width,
			final int height) {
