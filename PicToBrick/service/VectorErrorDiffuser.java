...
implements Quantizer {
	    ...
	    for (mosaicRow = 0; mosaicRow < mosaicHeight; mosaicRow++){
			...
			for (int mosaicColumn = 0; mosaicColumn < mosaicWidth; mosaicColumn++){
				...
				if (!(mosaicColumn == mosaicWidth-1)){
					workingMosaic[mosaicRow][mosaicColumn+1] = errorDistribution(workingMosaic[mosaicRow][mosaicColumn+1], (7.0/16.0), errorVector);
				}
				if (!(mosaicRow == mosaicHeight-1)){
					//pixel bottom: 5/16
					workingMosaic[mosaicRow+1][mosaicColumn] = errorDistribution(workingMosaic[mosaicRow+1][mosaicColumn], (5.0/16.0), errorVector);
					if (!(mosaicColumn == mosaicWidth-1)){
						//pixel bottom right: 1/16
						workingMosaic[mosaicRow+1][mosaicColumn+1] = errorDistribution(workingMosaic[mosaicRow+1][mosaicColumn+1], (1.0/16.0), errorVector);
					}
					if (!(mosaicColumn == 0)){
						//pixel bottom left: 3/16
						workingMosaic[mosaicRow+1][mosaicColumn-1] = errorDistribution(workingMosaic[mosaicRow+1][mosaicColumn-1], (3.0/16.0), errorVector);
					}
				}
			}
		}
	 ...
	 * description:      computes a color vector with the error vector and a special factor
	 ...
	 */
	private Lab errorDistribution(Lab color, double factor, Lab error){
		Lab result = new Lab();
		...
		if (result.getL() >  100.0){result.setL( 100.0);}
		if (result.getL() <    0.0){result.setL(   0.0);}
		if (result.getA() >  128.0){result.setA( 128.0);}
		if (result.getA() < -128.0){result.setA(-128.0);}
		if (result.getB() >  128.0){result.setB( 128.0);}
		if (result.getB() < -128.0){result.setB(-128.0);}
		return result;
	}
